...
def main(df, differentiate_bool=False, eth=False):
    ...
    df1 = getEvents(stationary_series['price'], events, [0.75, 0.75], 2.5 * sigma, 0.004, 48, t1, None, eth=eth)
    ...
    features_list = df1[["volatility", "ret_last_10", "iv_last_trade", "price_mark_price_deviation", "volume_last_10_trades", "buy_last_10_trades"]]

    print(df1.groupby("bin").size())

    
    '''
    This could be used for feature sampling
    
    model = RandomForestClassifier(n_estimators=100, criterion='entropy', bootstrap=False, class_weight='balanced_subsample', min_weight_fraction_leaf=0.05, n_jobs = 1)

    best_performer_per_feature = generate_model_proposals(model = model,
    ...
    def random_forest(i=2, avgU=avgU):
        ...
        df2_training, df2_validation = simple_stats(model = model)
    ...
    def simple_stats(model,df1=df1):
        ...
        print(f"Walk forward test results in {evaluate_model(model, X_train, y_train)}")
