...
class Quant_Strategy_5(bt.Strategy):
    ...
    def next(self):
        ...
        if self.data.datetime.time() >= datetime.time(10, 15) and self.data.datetime.time() <= datetime.time(15, 45) and self.roc_rsi[0] <= 30 and self.position.size == 0 and math.log(self.data1.close[-1] / self.data1.close[-7]) < math.log(self.data1.close[-1] / self.data1.close[-101]):
            self.buy(data = self.data, price = self.high_morning_range , exectype= bt.Order.Stop, valid = datetime.timedelta(minutes = 15))
        ...
        if self.data.datetime.time() >= datetime.time(10, 15) and self.data.datetime.time() <= datetime.time(15, 45) and self.position.size > 0:#self.position.size > 0:#
            self.close(data = self.data0, price = self.low_morning_range, exectype= bt.Order.StopLimit, plimit= self.low_morning_range, valid = datetime.timedelta(minutes= 15))
