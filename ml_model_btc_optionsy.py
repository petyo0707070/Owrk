...
def main(df, differentiate_bool = False, eth = False):
    ...
    df1 = getEvents(stationary_series['price'], events, [0.125, 0.25], 2 * sigma, 0.004, 48, t1, None, eth = eth)
    ...
    df1['buy'] = df1["t1"].apply(lambda x: 1 if df.loc[x, "direction"] == 'buy' else 0)
    df1['price_mark_price_deviation'] = df1["t1"].apply(lambda x: (df.loc[x, "price"] - df.loc[x, 'mark_price'])/df.loc[x, 'price'] )
    df1['volume_last_10_trades'] = df1["t1"].apply(lambda x: df['contracts'].loc[float(int(x) - 10): x].sum())
    #df1['buy_last_10_trades'] = df1["t1"].apply(lambda x: (df['direction'].iloc[x - 10: (x)]).mean())
    ...
    X_train, X_test, y_train, y_test = train_test_split(df1[['volatility', 'ret last 5', 'iv_last_trade', 'buy', 'price_mark_price_deviation', 'volume_last_10_trades']], df1['bin'], test_size=0.3, shuffle=False)
    ...
    def random_forest(i=0, avgU=avgU):
        ...
        df2_test = simple_stats(y_pred)

        if differentiate_bool == True:
            results_from_raw_data(df2_test, df)
    ...
    def simple_stats(y_pred, df1=df1):
        ...
        # This extra column is needed as due to the bid-ask spread sometimes the value the return column turns into a loss
        # even though we are on the right side of the market
        # Keep in mind a bid/ask spread of 0.001 is assumed if price is above 0.01 and 0.0005 if price below 0.01
        #df2['strategy_return'] = df2['ret'] * df2['bin']

        print(f"Accuracy is {df2['bool'].mean()}")

        df2['result'] = np.where(df2['prediction'] == 1, (df2['ret'] * 1+ 1), np.where(df2['prediction'] == - 1 , ( (df2['ret']*- 1 )+ 1) , 1 ))
...
def applyPtSlOnT1(close, events, ptSl, molecule, eth = False):
    ...
    for loc, t1 in events_['t1'].items():  # fillna(close.index[-1]).items():
        ...
        df2 = close.iloc[max(starting_row - 5, 0): starting_row ]
        ...
        if ((df0 > pt[loc]).any() and (df0 < sl[loc]).any() and (df0 < pt[loc]).idxmax() < (df0 > sl[loc]).idxmax()):
            ...
            if eth == False:
                out.loc[loc, 'fees'] = 0.001/close.loc[loc] if close.loc[index_pt_hit] > 0.01 else 0.0005/close.loc[loc]
            else:
                out.loc[loc, 'fees'] = 0.001/close.loc[loc] if close.loc[index_pt_hit] > 0.005 else 0.0005/close.loc[loc]
...
if __name__ == '__main__':
    # Normal Test
    df = pd.read_csv("eth_17_01_call_3800.csv", parse_dates=True)

    df = df[['timestamp', 'tick_direction', 'price', 'mark_price', 'iv', 'index_price', 'direction', 'contracts']][0:8000]
