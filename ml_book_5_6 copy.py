...
def main(df):
    ...
    t1 = stationary_series['close'].index.searchsorted(events + pd.Timedelta(hours = 6))
    ...
    def random_forest(i = 0, avgU = avgU):
        ...
        if i == 0:        
            ...
            model = RandomForestClassifier(n_estimators= 1, criterion= 'entropy', bootstrap=False, class_weight='balanced_subsample', min_weight_fraction_leaf=0.05)#, max_features= int(1))
        ...
        frequency = (end - start).days / 365.25
...
def plotMinFFD(df):
    ...
    for d in np.linspace(0,1,11):
        df1=np.log(df[['close']]).resample('15min').last() # downcast to daily obs
...
def applyPtSlOnT1(close,events,ptSl,molecule):
    ...
    for loc,t1 in events_['t1'].items():#fillna(close.index[-1]).items():
        ...
        starting_row = close.index.get_loc(loc)
        ...
        df2 = (df2/ close.iloc[starting_row - 6] - 1)
        return_last_5 = df2.iloc[-1]
        bullish_last_5 = (close.iloc[max(starting_row - 6, 0): starting_row].diff() > 0).sum()
...
def getDailyVol(close,span0=100):
    ...
    df0=close.index.searchsorted(close.index-pd.Timedelta(minutes=5))
    print(df0)
    df0=df0[df0>0]
...
if __name__=='__main__':
    # Normal Test
    df = pd.read_csv('btc_sampled_tick_data.csv',  parse_dates= True, names =['time','close', 'high', 'low', 'tick_ratio', 'buy_ratio', 'log_return','open'], header = 1)
    df = df.dropna()
