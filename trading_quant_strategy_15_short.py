...
##########################################################
#Linda Rashcke's ORB Strategy, this is the full version as I understand it
##########################################################
...
class Quant_Strategy_5(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.volatility_6_day = math.log(self.data1.close(-1) / self.data1.close(-7))
        self.volatility_100_day = math.log(self.data1.close(-10) / self.data1.close(-101))
    ...
    def next(self):
        ...
        #print(f"{self.data.datetime.datetime()}, position: {self.position.size}")
        ...
        if self.data.datetime.time() >= datetime.time(10, 15) and self.data.datetime.time() <= datetime.time(15, 45) and self.roc_rsi[0] >= 70 and self.position.size == 0:
            self.sell(data = self.data, price = self.low_morning_range , exectype= bt.Order.Stop, valid = datetime.timedelta(minutes = 15))
        ...
        if self.data.datetime.time() >= datetime.time(10, 15) and self.data.datetime.time() <= datetime.time(15, 45) and self.position.size < 0:#self.position.size > 0:#
            self.close(data = self.data0, price = self.high_morning_range, exectype= bt.Order.StopLimit, plimit= self.high_morning_range, valid = datetime.timedelta(minutes= 15))
        
        # Checks whether the position is profitable by the end of the trading session, if so it is held overnight, if not it is closed
        if self.data.datetime.time() == datetime.time(16, 00) and self.position.size < 0 and self.position.price < self.data.close[-1]:
            self.close(data = self.data0, exectype = bt.Order.Market)

        # Closes an overnight position if it the open moved in its favour
        if self.data.datetime.time() == datetime.time(10, 15) and self.position.size < 0 and self.data0.close[0] - self.data0.open[-3] > 0:
