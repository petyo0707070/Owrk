...
######################################################################
#Very strong results, pretty much the best ones so far but need to be traded long-term
# Simplistic for everything except equities and bitcoin it tends to very well with long + short
...
def main():
    ...
    def run_strategy(datastream, timeframe, leverage):
        ...
        df['volume'] = 1
        df_training = df[round(0.6 * len(df)):]
        print(df_training)
    ...
    run_strategy('trading_view_spy_daily.csv', timeframe = 1, leverage = 100)
...
class Quant_Strategy_5(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.roc_fast = (self.data.close(0) - self.data.close(-self.p.fast))/self.data.close(-self.p.fast)
        self.roc_slow = (self.data.close(0) - self.data.close(-self.p.slow))/self.data.close(-self.p.slow)
        ...
        self.sell_signal = bt.And(self.roc_fast(0) > self.roc_slow(0), self.roc_fast(-1) > self.roc_slow(-1))

        self.long_exit = bt.And(self.roc_fast(0) > self.roc_slow(0), self.roc_fast(-1) > self.roc_slow(-1))
        self.short_exit = bt.And(self.roc_fast(0) < self.roc_slow(0), self.roc_fast(-1) < self.roc_slow(-1))
